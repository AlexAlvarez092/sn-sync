...
export interface SnAuthConfigRuntime extends SnBaseCommandRuntime {
...
const defaultRuntime: SnAuthConfigRuntime = {
  ...defaultBaseRuntime,
...
export async function runSnAuthConfigCommand(
  context: vscode.ExtensionContext,
  authService: SnAuthConfigCommandServiceApi,
  runtime: SnAuthConfigRuntime = defaultRuntime,
): Promise<void> {
  ...
  if (!authInput) {
    void runtime.showInformationMessage(SN_SYNC_MESSAGES.AUTH_CONFIG_CANCELLED);
    return;
  }

  try {
    await authService.saveAuth(context, workspaceFolderUri, authInput);
    void runtime.showInformationMessage(SN_SYNC_MESSAGES.AUTH_CONFIG_SUCCESS);
  } catch (error) {
    showPrefixedCommandError(
      runtime,
      SN_SYNC_MESSAGES.AUTH_CONFIG_FAILED_PREFIX,
      error,
      {
        code: SN_SYNC_ERROR_CODES.AUTH_CONFIG_FAILED,
...
export function registerSnAuthConfigCommand(
  context: vscode.ExtensionContext,
  authService: SnAuthConfigCommandServiceApi = new SnAuthService(),
): void {
  registerCommandWithStatus({
    context,
    commandId: SN_SYNC_COMMANDS.AUTH_CONFIG,
    task: () => runSnAuthConfigCommand(context, authService),
