...
import {
  registerSnResetCommand,
  runSnResetCommand,
} from "@commands/snResetCommand.js";
import {
  SN_SYNC_COMMANDS,
  SN_SYNC_MESSAGES,
} from "@shared/constants/snSyncConstants.js";

suite("snResetCommand", () => {
  ...
  test("shows info when quick pick is cancelled", async () => {
    ...
    assert.deepStrictEqual(shownInfos, [SN_SYNC_MESSAGES.RESET_CANCELLED]);
  });

  test("dispatches to reset auth", async () => {
    ...
    assert.deepStrictEqual(executedCommands, [SN_SYNC_COMMANDS.RESET_AUTH]);
  });

  test("dispatches to reset index", async () => {
    const executedCommands: string[] = [];
    ...
    assert.deepStrictEqual(executedCommands, [SN_SYNC_COMMANDS.RESET_INDEX]);
  });

  test("register callback executes reset command", async () => {
    const shownErrors: string[] = [];
    const context = {
      subscriptions: [] as vscode.Disposable[],
    } as unknown as vscode.ExtensionContext;
