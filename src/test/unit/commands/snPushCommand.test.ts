...
suite("snPushCommand", () => {
  ...
  test("register callback executes push command", async () => {
    const shownErrors: string[] = [];
    const context = {
      subscriptions: [] as vscode.Disposable[],
    } as unknown as vscode.ExtensionContext;
...
async function withPatchedWindowShowQuickPick(
  ...
  const original = windowObject.showQuickPick;
  windowObject.showQuickPick = showQuickPick;
  try {
    await run();
  } finally {
    windowObject.showQuickPick = original;
  }
}
...
async function withPatchedCommandsExecuteCommand(
  ...
  const original = commandsObject.executeCommand;
  commandsObject.executeCommand = executeCommand;
  try {
    await run();
  } finally {
    commandsObject.executeCommand = original;
  }
}
