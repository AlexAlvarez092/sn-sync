...
- Entry point: src/commands/snAuthConfigCommand.ts
...
- Normally through `sn: auth` -> `configure auth`.
- When testing or invoking the internal delegate directly.
...
10. If authInput is undefined, command shows SN_SYNC_MESSAGES.AUTH_CONFIG_CANCELLED and exits.
11. If input is complete, call authService.saveAuth(context, workspaceFolderUri, authInput).
12. On success, show SN_SYNC_MESSAGES.AUTH_CONFIG_SUCCESS.
13. On failure, show SN_SYNC_MESSAGES.AUTH_CONFIG_FAILED_PREFIX + normalized error.
...
- SN_SYNC_MESSAGES.AUTH_CONFIG_CANCELLED for user cancellation/invalid input.
- SN_SYNC_MESSAGES.AUTH_CONFIG_FAILED_PREFIX for save failures.
...
sequenceDiagram
	...
	participant C as sn: auth config command
	...
	alt No workspace
		...
		alt Any required value canceled or empty
			C->>R: showInformationMessage(AUTH_CONFIG_CANCELLED)
		else Complete auth payload
			C->>S: saveAuth(context, workspaceFolderUri, authInput)
			alt Success
				C->>R: showInformationMessage(AUTH_CONFIG_SUCCESS)
			else Failure
				C->>R: showErrorMessage(AUTH_CONFIG_FAILED_PREFIX + error)
			end
			end
		end
	end
...
- Symptom: Command exits with "sn-sync auth config cancelled"
  - Cause: One input was canceled or blank.
  - Resolution: Rerun command and complete all fields.

- Symptom: "Failed to save sn-sync auth config"
...
- Symptom: OAuth flow fails during token exchange/refresh
  ...
  - Resolution: Rerun `sn: auth`, choose `configure auth`, complete sign-in again, then use `validate auth`.
